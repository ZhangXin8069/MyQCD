...
我们感谢 Lucius Bushnaq 与 Yannick Kluth 在核对费曼规则与总体框架方面的帮
助，感谢 Jonas Klappert 在使用 \texttt{FireFly} 时的支持、Johann Usovitsch
在使用 \texttt{Kira} 时的支持、Alexander Voigt 就一般技术问题的讨论，以及
Mauro Papinutto 指出本手稿早期版本中的一处笔误。本工作由德意志研究联合会
（\textit{Deutsche Forschungsgemeinschaft, DFG}）项目
\href{http://gepris.dfg.de/gepris/projekt/386986591}{HA 2990/9-1} 资助，并由
美国能源部奖项 DE-SC0008347 支持。本文档的准备使用了美国能源部科学办公室
HEP 用户设施——费米国家加速器实验室（Fermilab）的资源。Fermilab 由
Fermi Research Alliance, LLC（FRA）依据合同号 DE-AC02-07CH11359 管理。部分计
算使用了 \abbrev{RWTH} Aachen 大学项目 \textit{rwth0244} 授予的计算资源。本
文所有费曼图均使用 Ti\textit{k}Z-Feynman\,\cite{Ellis:2016jkw} 绘制。
...
本节给出我们的计算所需的全部费曼规则。所有动量均视为外向。$\xi$
是通常的 \qcd\ 规范参数，$\kappa$ 是流方程 \noeqn{eq:flow} 中引入的附加梯度
流规范参数。带实心箭头的线为流线；带虚线箭头的线既可为流线亦可为流动传播
子。符号 $p,q,r$ 表示欧氏四动量，$s,t$ 为实值流时间变量。
...
\textbf{辅助费曼规则}\\
下列费曼规则是 \sct{sec:implementation} 所述实现中的辅助规则。它们表示带四条
胶子或胶子流线的顶点，使我们能把所有费曼图的色结构因式分解。
...
\section{解析结果}\label{app:analytic}

此处我们给出能够获得的色系数解析结果。对 \noeqn{eq:econst} 中 $e_{2,0}$ 的
$\TF\CA$ 项，我们得到
\begin{equation}
  \begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
为多重多对数函数（multiple polylogarithms）\,\cite{Goncharov:1998kja,Borwein:1999js}。

\noeqn{eq:c2} 中 $C_2$ 的 $\CF\TF$ 系数为
\begin{equation}
  \begin{split}
...
最后，\noeqn{eq:sconst} 中 $s_{2,0}$ 的 $\CF\TF$ 系数为
\begin{equation}
  \begin{split}
    -(15.7975 \dots) =
